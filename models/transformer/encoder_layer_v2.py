...
class EncoderBlockWithNewTechniques(nn.Module):
    def __init__(self, d_model: int, num_heads: int, d_ff: int, dropout: float):
        ...
        self.residuals = nn.ModuleList([ResidualConnectionWithRMSNorm(d_model, dropout) for _ in range(2)])
...
if __name__ == "__main__":
    ...
    d_ff = 2048
    dropout = 0.1
    encoder_block = EncoderBlockWithNewTechniques(d_model, num_heads, d_ff, dropout)
    batch_size = 5
    seq_len = 10
