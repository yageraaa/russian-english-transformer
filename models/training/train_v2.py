...
from models.transformer.transformer_v2 import TransformerWithNewTechniques
...
def train_model(cfg: DictConfig):
    ...
    for epoch in epoch_progress:
        epoch_progress.set_description(f"Эпоха {epoch + 1}/{cfg.training.num_epochs}")
        model.train()
...
def log_translations(model, tokenizer, device, cfg: DictConfig, epoch: int):
    ...
    with torch.no_grad():
        for src, ref in examples:
            ...
            print(f"Эпоха {epoch} - Исходный текст: {src}, Эталон: {ref}, Перевод: {translation}")
            translations.append([src, ref, translation])

    return {f"examples_epoch_{epoch}": wandb.Table(columns=["Исходный текст", "Эталон", "Перевод"], data=translations)}
